...
function volumeSeriesData(data) {
  return data.dates.map((time, i) => ({
    ...
    color: data.closes[i] >= data.opens[i] ? 'rgba(34,197,94,0.75)' : 'rgba(239,68,68,0.75)',
  }));
}
...
function histogramData(dates, values) {
  return dates.map((time, i) => {
    ...
    return {
      ...
      color: (value || 0) >= 0 ? 'rgba(34,197,94,0.75)' : 'rgba(239,68,68,0.75)',
    };
  });
}
...
async function renderLightweightChart(containerId, fallbackId, symbol, interval, metaPrefix = '') {
  ...
  try {
    ...
    const data = await fetchJson(`/api/chart_history?symbol=${encodeURIComponent(symbol)}&interval=${encodeURIComponent(interval)}`);
    ...
    setLegendContent(panes.volumeLegend, [
      { label: '成交量', value: lastVolume != null ? lastVolume.toLocaleString('ja-JP') : '--', color: '#22c55e' },
      { label: '量比', value: data.volume_ratio != null ? `${data.volume_ratio.toFixed(2)}x` : '--', color: '#a78bfa' },
    ]);
    setLegendContent(panes.macdLegend, [
      ...
      { label: 'Hist', value: lastHist != null ? lastHist.toFixed(4) : '--', color: lastHist >= 0 ? '#22c55e' : '#ef4444' },
    ]);
