...
function updateUniverseHeader(label) {
  ...
  if (pill) pill.textContent = `Current Universe: ${label || '--'}`;
}
...
function renderScreenerResults(data) {
  ...
  if (!items.length) {
    return warning + '<div class="ai-placeholder">No matches right now. Try another strategy, expand the universe, or refresh later.</div>';
  }

  return `
    ${warning}
    <div class="screener-result-meta">
      <span>Universe: ${data.universe || '--'} / ${data.universeSize || 0} stocks</span>
      <span>Matches: ${data.count || 0}</span>
    </div>
...
function renderScreenerCard(item) {
  ...
  const changePct = item.changePct != null ? `${item.changePct > 0 ? '+' : ''}${item.changePct.toFixed(2)}%` : '--';
  const changeCls = item.changePct > 0 ? 'up' : (item.changePct < 0 ? 'down' : 'neutral');
  const volumeSvg = volumeBars(item.volumes20 || [], 220, 48);
  const macdHist = macdBars(item.histSeries || [], 220, 58);
  const macdPath = linePath(item.macdSeries || [], 220, 58);
  const signalPath = linePath(item.signalSeries || [], 220, 58);
  const candles = renderCandles(item.opens20 || [], item.highs20 || [], item.lows20 || [], item.closes20 || [], 220, 76);
  const ma20Series = Array.isArray(item.ma20Series) && item.ma20Series.length ? item.ma20Series : (item.closes20 || []);
  const ma20Path = linePath(ma20Series, 220, 76);
  const isAdding = ScreenerPage.adding.has(item.symbol);
  const isSaved = !!item.inWatchlist;
  const disabled = isAdding ? 'disabled' : '';
  const btnLabel = getScreenerWatchButtonLabel(isSaved, isAdding);
  const safeName = (item.name || item.symbol).replace(/'/g, "\\'");
  const chartUrl = `/chart?symbol=${encodeURIComponent(item.symbol)}&name=${encodeURIComponent(item.name || item.symbol)}`;

  return `
    <div class="screener-result-card" role="button" tabindex="0" onclick="openScreenerChart('${item.symbol}', '${safeName}')" onkeydown="handleScreenerCardKey(event, '${item.symbol}', '${safeName}')">
      ...
      <div class="screener-chart-grid">
        <div class="screener-chart-card screener-chart-main">
          <div class="screener-chart-title">Candles + MA20</div>
        ...
        <div class="screener-chart-card">
          <div class="screener-chart-title">Volume</div>
      ...
      <div class="screener-metrics">
        ...
        <span>Signal: ${item.macdSignal ?? '--'}</span>
        <span>Volume Ratio: ${item.volumeRatio ?? '--'}x</span>
        <span>MA20: ${item.ma20 ?? '--'}</span>
      </div>

      <div class="screener-actions">
        <span class="screener-card-tip">Open the chart from the card, or use the button to manage the watchlist.</span>
        <button
        ...
        >${btnLabel}</button>
        <a class="page-link" href="${chartUrl}" onclick="event.stopPropagation()">View Details</a>
      </div>
    </div>
  `;
}
...
function getScreenerWatchButtonLabel(isSaved, isAdding) {
  if (isAdding) return isSaved ? 'Removing...' : 'Adding...';
  return isSaved ? 'In Watchlist' : 'Add to Watchlist';
}
...
async function toggleScreenerWatch(symbol, name, isSaved) {
  ...
  try {
    ...
    alert(`${isSaved ? 'Remove failed' : 'Add failed'}: ${e.message}`);
    syncScreenerWatchButtons(symbol, isSaved, false);
  } finally {
...
async function loadScreenerResults() {
  ...
  body.innerHTML = '<div class="ai-loading"><div class="ai-spinner"></div>Refreshing screen results, please wait...</div>';
  try {
    const data = await fetchApiJson(`/api/screener?mode=${encodeURIComponent(ScreenerPage.mode)}&universe=${encodeURIComponent(ScreenerPage.universe)}&limit=18`);
    body.innerHTML = renderScreenerResults(data);
    if (lastFetch && data.updated) lastFetch.textContent = `Updated: ${data.updated}`;
  } catch (e) {
    body.innerHTML = `<div class="ai-placeholder" style="color:var(--down)">Failed to load screen results: ${e.message}</div>`;
  }
