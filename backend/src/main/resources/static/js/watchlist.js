...
function wlMarkerColor(q) {
  const color = String(q?.markerColor || '').trim().toLowerCase();
...
function wlSparklineSvg(q) {
  const up = q.change > 0;
  const down = q.change < 0;
  const sparkColor = up ? '#ef4444' : (down ? '#22c55e' : '#6b7280');
...
function renderWatchlistDetail(symbol) {
  ...
  const down = q.change < 0;
  const cls = up ? 'up' : (down ? 'down' : 'neutral');
  const marketValue = q.marketValue != null ? fmtK(q.marketValue) : '--';
...
async function saveWatchlistStatus() {
  ...
  const saved = await persistWatchlistPortfolio(portfolio => {
    const idx = portfolio.findIndex(item => item.symbol === symbol);
...
async function saveWatchlistMarker(color) {
  ...
  const currentQuotes = Array.isArray(WatchlistState.quotes) ? [...WatchlistState.quotes] : [];
  const target = currentQuotes.find(item => item.symbol === symbol);
  if (target) target.markerColor = nextColor;
  renderWatchlist(currentQuotes);
  const saved = await persistWatchlistPortfolio(portfolio => {
    const idx = portfolio.findIndex(item => item.symbol === symbol);
    if (idx < 0) return false;
    portfolio[idx].markerColor = nextColor;
    return true;
  });
  if (!saved) {
    renderWatchlist(WatchlistState.quotes);
...
async function refreshWatchlistData() {
  const [quotes, indexes] = await Promise.all([
    fetchApiJson('/api/quotes'),
    fetchApiJson('/api/index_quotes'),
  ]);
...
function initWatchlist() {
  ...
  document.querySelectorAll('.wl-menu-item').forEach(item => {
    item.addEventListener('click', async event => {
      ...
      if (action === 'clear-marker') await saveWatchlistMarker('');
      if (action === 'watch') {
        await addToWatchFromMenu();
      }
    });
  });
  root.querySelectorAll('[data-marker-color]').forEach(btn => {
    btn.addEventListener('click', async event => {
      ...
      await saveWatchlistMarker(current === picked ? '' : picked);
    });
  });
